...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-md p-4 lg:p-6 mb-6 border border-gray-200">
        <form action="{{ route('admin.workers') }}" method="GET" class="flex flex-col lg:flex-row gap-3 lg:gap-4">
            <input type="text" name="search" placeholder="Cari worker..."
                    ...
                    @forelse($workers as $worker)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <div
                                        class="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-[#052c62] text-white flex items-center justify-center font-bold">
                            ...
                            <td class="px-4 py-3 text-center font-bold text-green-600 hidden sm:table-cell">
                                Rp {{ number_format($worker->price_per_hour, 0, ',', '.') }}
                            </td>
                            <td class="px-4 py-3 text-center">
                                @if($worker->approval_status == 'approved')
                                    <span
                                        class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-semibold">Approved</span>
                                ...
                                @else
                                    <span
                                        class="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs font-semibold">Rejected</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-center">
